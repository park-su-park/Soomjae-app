...
actual class FirebasePushNotificationService(
    ...
    actual override fun observeDeviceId(): Flow<String?> = flow {
        try {
            val deviceId = Firebase.installations.id.await()
            logger.info(TAG, "Firebase Installation ID received: $deviceId")
            emit(deviceId)
        } catch (e: Exception) {
            currentCoroutineContext().ensureActive()
            logger.error(TAG, "Failed to get Firebase Installation ID", e)
            emit(null)
        }
    }
}
